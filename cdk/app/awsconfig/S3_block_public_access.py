...
# Main Code #
#############
def get_resource(client, configuration_item):
    return client.get_resource(TypeName=configuration_item["resourceType"], Identifier=configuration_item["resourceId"])

def resource_to_cftemplate(ccapi_objects):
    d = {'Resources': {}}
    for ccapi_object in ccapi_objects:
        id = ccapi_object['ResourceDescription']['Identifier']
        d['Resources'][id] = {}
        d['Resources'][id]['Type'] = ccapi_object['TypeName']
        d['Resources'][id]['Properties'] = json.loads(ccapi_object['ResourceDescription']['Properties'])
    return d
...
def lambda_handler(event, context):
    ...
    try:
        ...
        AWS_CCAPI_CLIENT = get_client('cloudcontrol', event)
        if invoking_event['messageType'] in ['ConfigurationItemChangeNotification', 'ScheduledNotification', 'OversizedConfigurationItemChangeNotification']:
            configuration_item = get_configuration_item(invoking_event)
            print(configuration_item)
            if is_applicable(configuration_item, event):
                compliance_result = evaluate_compliance(AWS_CCAPI_CLIENT, event, configuration_item, valid_rule_parameters)
